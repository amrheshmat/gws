	...
	public class SpecialRequestController : Controller
	{
		...
		public async Task<Response> Add([FromBody] SpecialRequest specialRequestModel)
		{
			...
			if (SpecialRequest != null)
			{
				...
				decimal? permissionId = _repo.Filter<Permission>(permission => permission.permissionArea == "SpecialRequest").FirstOrDefault().permissionId;
				if (permissionId != null)
				{
					...
					if (role != null)
					{
						...
						{
							User user = new User();
							user = _repo.Filter<User>(e => e.roleId == rolePermission.roleId).FirstOrDefault();
							users.Add(user);
						}
					}
				}
